...
class ApplyVertCol(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if 'shine' not in me.color_attributes:
            vcolor_layer = me.color_attributes.new(name='shine', type='BYTE_COLOR', domain='CORNER')
            v = 0
        ...
        if 'opacity' not in me.color_attributes:
            vcolor_layer = me.color_attributes.new(name='opacity', type='BYTE_COLOR', domain='CORNER')
            v = 0
            for poly in me.polygons:
                for idx in poly.loop_indices:
                    vcolor_layer.data[v].color = (0, 0, 0, 1)
        ...
        for face in me.polygons:
            if face.index in face_selection:
                for loop_idx in face.loop_indices:
                        ...
                        me.color_attributes['shine'].data[loop_idx].color = (shine/31,) * 3 + (1,)
                        me.color_attributes['opacity'].data[loop_idx].color = (transl,) * 3 + (1,)
