...
def import_mixamo(context, filepath, target_rig_name, new_action_name):
    ...
    if target_rig.animation_data.action_slot is None:
        # Check for suitable existing slots
        if len(target_rig.animation_data.action_suitable_slots) > 0:
            target_rig.animation_data.action_slot = target_rig.animation_data.action_suitable_slots[0]
        else:
    ...
    slot = target_rig.animation_data.action_slot
    ...
    for axis in [0, 1, 2]:
        ...
        fc = fcurves.new(data_path=dp, index=axis)
