...
def pack_object_textures(meshes, texture_path):
    ...
    for mesh in meshes:
        texture_rects |= {
            (poly.x, poly.y, poly.tex_width, poly.tex_height)
            for poly in mesh.polygons
            if poly.tex_width > 0 and poly.tex_height > 0
        }

    if not texture_rects:
        new_texture_map = np.zeros((1, 1, 4), dtype='uint8')
    ...
    if new_pos is None or new_map_width <= 0 or new_map_height <= 0:
        uvtable = {(x0, y0, w, h): (x0, y0) for (x0, y0, w, h) in texture_rects}
        return uvtable, wad_texture_map
