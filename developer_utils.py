...
def setup_addon_modules(path, package_name, reload):
    ...
    def get_submodule_names(path = path[0], root = ""):
        module_names = []
        # Skip dev/test scripts that execute at import time.
        skip_prefixes = ("test_", "hex_")
        skip_names = {"find_chunks"}
        for importer, module_name, is_package in pkgutil.iter_modules([path]):
            if module_name in skip_names or module_name.startswith(skip_prefixes):
                continue
            if is_package:
