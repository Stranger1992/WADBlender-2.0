...
class WadBlenderAddShineVertexLayer(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        vcolor_layer = context.active_object.data.color_attributes.new(name='shine', type='BYTE_COLOR', domain='CORNER')
        v = 0
...
class WadBlenderAddOpacityVertexLayer(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        vcolor_layer = context.active_object.data.color_attributes.new(name='opacity', type='BYTE_COLOR', domain='CORNER')
        v = 0
        for poly in context.active_object.data.polygons:
            for idx in poly.loop_indices:
                vcolor_layer.data[v].color = (0, 0, 0, 1)
...
class WadBlenderAddShadeVertexLayer(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        v = 0
        vcolor_layer = context.active_object.data.color_attributes.new(name='shade', type='BYTE_COLOR', domain='CORNER')
        for poly in context.active_object.data.polygons:
            for idx in poly.loop_indices:
...
class WadBlenderAddMaterial(bpy.types.Operator, ExportHelper):
    ...
    def execute(self, context):
        ...
        try:
            ...
            self.report({'WARNING'}, f"Could not save texture to {self.filepath}: {str(e)}")
...
class ObjectPanel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        shade_missing = 'shade' not in context.active_object.data.color_attributes
        shine_missing = 'shine' not in context.active_object.data.color_attributes
        opacity_missing = 'opacity' not in context.active_object.data.color_attributes
