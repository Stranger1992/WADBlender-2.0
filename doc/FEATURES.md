...
- Mixamo animation import and retargeting to the `LARA_RIG` armature (the importer currently requires this exact object name; not configurable yet)
...
- Importing multiple WADs into one .blend can create name conflicts (duplicate object/collection names or texture page reuse) because there is no automatic deduplication; import into separate files, or prefix/rename imported collections and texture pages before bringing in a second WAD.
