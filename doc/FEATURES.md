...
- TombEngine: prefer texture pages (skip packing) to preserve original UV mapping; auto-packing can alter UVs.  
  - When importing, the add-on will look for optional PBR-style textures placed next to the baked texture or the original WAD using these suffixes: `_diffuse`/`_albedo`/`_base_color`, `_emission`/`_emit`, `_roughness`/`_rough`, `_specular`/`_spec`.  
  - It also reads sidecar `*.xml` MaterialData files (from Tomb Editor/WadTool) to locate external texture paths (Color, Emissive, Specular, Roughness).  
  - Any maps found are wired into Principled BSDF (base color + alpha, emission, roughness, specular). Missing maps simply fall back to the baked diffuse.
